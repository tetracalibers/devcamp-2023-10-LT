...
好きな画像にフィルタを重ねて適用して結果をダウンロードできる、画像加工アプリを作っています。
...
2枚の一時レイヤーを用意し、交互に使うことでフィルタの適用を実現します。
...
明るくした画像を白黒にする前に、近隣のピクセルをランダムに参照して重ね合わせることで、色の濃い部分を分散させます。
そうすることで、より鉛筆らしい淡さが出せます。
...
まずは、エッジ付近に色を乗せて、黒鉛筆の線を色鉛筆に近づけます。

mix関数は、第3引数に割合を指定して、2つの色を混ぜ合わせる関数です。第3引数が大きい、つまりエッジが強いほど、第2引数の色が強くなります。

このとき、そのままの色をただ乗せてしまうと、鉛筆の質感がなくなってしまうので、元画像の色を加工したsprayColorとpastelColorを第2引数に指定しています。
...
ランダムな色のボロノイを少し暗めにしたものを、overlayブレンドモードで合成します。
...
シェーダでボロノイを計算する手法では、座標を拡大してからボロノイ分割を行うことになるので、実際の座標の色をボロノイ図に反映させることは難しいものでした。
...
移動モードをONにし、画像をドラッグすると、フィルタを適用する範囲を変更できます。
半径はスライダーで調整できます。
