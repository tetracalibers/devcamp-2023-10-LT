...
GLSLで画像加工フィルタを書いて、WebGLで動かすという実験の中で、簡単な画像加工アプリを作りました。
...
内部実装の整理ができていないので、やや動作が重く、スマホにはまだ対応していません。
...
画像の各点から色を取り出し、数式で加工していきます。

今日はその、GLSLでの色の加工について話していきたいと思います。
...
エッジであれば線を描き、そうでなければ領域の色で塗るように条件分岐しています。
...
一旦画像を小さなレイヤーに縮小して描画し、それをgl.NEARESTテクスチャパラメータを指定した上で元のサイズのレイヤーに写すことで、ニアレストネイバー法で拡大され、簡単にモザイク画像がつくれます。
...
また、円錐を利用してボロノイを生成することで、各領域を、その領域内のいずれかの点の元画像の色で塗りつぶすことができ、少し変わったモザイクになります。

しかし、円錐の数だけバッファを確保してしまっているので、モザイクの細かさを気軽に変えられないのが難点です。
