...
そして、明るくした画像を白黒にしたものと、エッジを抽出した画像を乗算合成します。↪︎

明るくした画像を白黒にする前に、近隣のピクセルをランダムに参照して↪︎重ね合わせることで、色の濃い部分を分散させます。
そうすることで、より鉛筆らしい淡さが出せます。

今はエッジの強い部分が白く、それ以外は黒くなってしまっているので、↪︎白黒反転させて、エッジのみを黒く残します。
...
ここでは、エッジの強さに応じて、元画像の色を混ぜ込んでいます。↪︎

このとき、そのままの色をただ乗せてしまうと、鉛筆の質感がなくなってしまうので、元画像の色を加工したsprayColorとpastelColorを使用しています。
